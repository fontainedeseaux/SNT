...
def synthese_additive():
    
    def dec_vers_hex(x):
        resultat = hex(x).split('x')[-1]
        if len(resultat)!=1:
            return resultat
        return '0'+resultat
    ...
    def couleur(r,v,b):
        return f'#{dec_vers_hex(r)}{dec_vers_hex(v)}{dec_vers_hex(b)}'

    def decoupe(A,B, k):
        '''renvoie deux points M et N sous forme de liste tq : AM=k*AB et BN=k*BA'''
        coeff=[k, 1-k]
        return [[A[0]+coeff[i]*(B[0]-A[0]), A[1]+coeff[i]*(B[1]-A[1])] for i in range(2)]
            
    def droite(P1, P2):
        '''renvoie coeff direct et ordonnée à l'orig'''
        m = (P2[1]-P1[1])/(P2[0]-P1[0])
        p = P1[1]-m*P1[0]
        return m, p
        
    def croisement(P1, P2, P3, P4):
        '''renvoie les coordonnées des points d'intersections de (P1P2) avec (P3P4)'''
        m1, p1 = droite(P1,P2)
        m2, p2 = droite(P3, P4)
        x = (p2-p1)/(m1-m2)
        y = m1*x + p1
    ...
    def maj():
        ...
        try:
            red.get()
            if not (0<=red.get()<=255):
                red.set(0)
        except:
            red.set(0)
        try:
            green.get()
            if not (0<=green.get()<=255):
                green.set(0)
        except:
            green.set(0)
        ...
        try:
            blue.get()
            if not (0<=blue.get()<=255):
                blue.set(0)
        except:
            blue.set(0)
        
        canvas.itemconfigure(1, fill=couleur(red.get(),0,0))
        canvas.itemconfigure(2, fill=couleur(0,green.get(),0))
        canvas.itemconfigure(3, fill=couleur(0,0,blue.get()))
        canvas.itemconfigure(4, fill=couleur(red.get(),green.get(),blue.get()))
        canvas.after(100,maj)
    ...
    H = 2*d+cote*3**0.5/2
    H=round(1.25*H)
    root.geometry(f'{round(L)}x{round(H)}')
    root.title("SYNTHESE ADDITIVE COULEURS")
    #root.geometry('1000x1000')
    # triangle équilatéral. Chaque sommet aura une lampe
    R = [d, d]
    V = [L-d, d]
    B = [L/2, d+ cote*3**0.5/2]
    ...
    b=[B]
    k=1/4
    r.extend(decoupe(V,B,k))
    v.extend(decoupe(R,B,k))
    b.extend(decoupe(R,V,k))
    A=croisement(b[0],b[1],r[0],r[2])
    C=croisement(b[0],b[1],v[0],v[1])
    D=croisement(v[0],v[1],r[0],r[1])
    rouge = R+A+C+D

    E=croisement(b[0],b[2],r[0],r[1])
    F=croisement(v[0],v[2],b[0],b[2])

    vert = V+D+E+F

    G=croisement(v[0],v[2],r[0],r[2])
    bleu=B+A+G+F
    ...
    canvas.create_polygon(rouge, fill=couleur(red.get(),0,0), width=2)
    canvas.create_polygon(vert, fill=couleur(0,green.get(),0), width=2)
    canvas.create_polygon(bleu, fill=couleur(0,0,blue.get()), width=2)
    canvas.create_polygon(poly, fill=couleur(red.get(),green.get(),blue.get()), width=2)
    pas=5

    boite_rouge = Spinbox(root,from_=0,to=255,increment=pas,textvariable=red,width=5,fg=couleur(red.get(),0,0), command=maj)
    boite_rouge.configure(font=('Calibri',16))
    boite_rouge.pack(padx=30,pady=10)

    boite_verte = Spinbox(root,from_=0,to=255,increment=pas,textvariable=green,width=5,fg=couleur(0,green.get(),0),command=maj)
    boite_verte.configure(font=('Calibri',16))
    boite_verte.pack(padx=30,pady=10)

    boite_bleu = Spinbox(root,from_=0,to=255,increment=pas,textvariable=blue,width=5,fg=couleur(0,0,blue.get()),command=maj)
    boite_bleu.configure(font=('Calibri',16))
    boite_bleu.pack(padx=30,pady=10)
